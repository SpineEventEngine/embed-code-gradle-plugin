...
- Argument: $ARGUMENTS
  ...
  - `all`: sweep every project-owned file.
  - A path: scope the sweep to that directory or file.
- Scope: project-owned files only, per
  `.agents/guidelines/project-owned-files.md` — skip submodule contents.
...
- Report per the skill: Mode, FilesScanned, FilesChanged, Changes[] grouped
  by catalog topic (file, line, before → after), and Skipped[] (file, line,
  topic, reason).
