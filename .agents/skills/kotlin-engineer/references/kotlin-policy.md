...
- Build with the configured toolchain, but emit Java 17-compatible bytecode for
...
- Preserve the configured Kotlin language and API versions. The plugin runs
  with Kotlin supplied by Gradle; consumers must not need to install Kotlin or
...
- Use project-relative paths and Gradle abstractions instead of environment-
...
- Prefer Gradle managed properties for configurable values:
  `Property<T>`, `ListProperty<T>`, `MapProperty<K, V>`,
  `DirectoryProperty`, and `RegularFileProperty`.
...
- Do not introduce unrelated coroutine, Flow, multiplatform, Android,
  Protobuf, or framework conventions.
...
- Do not expose credentials, tokens, internal paths, or low-level exception
...
Use KDoc links only for symbols visible to the documented source set. Do not
link published API documentation to `.agents/`, `buildSrc`, issues, branches,
or transient implementation notes.
